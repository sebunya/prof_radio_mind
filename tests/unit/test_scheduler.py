...
def test_scheduler_default_no_jobs() -> None:
    ...
    with (
        ...
        patch.object(settings, "enable_nova_radoxo_collector", False),
        patch.object(settings, "enable_nova_radio_australia_collector", False),
        patch.object(settings, "enable_capital_collector", False),
        patch.object(settings, "enable_capital_ukradiolive_collector", False),
        patch.object(settings, "enable_nightly_reconciliation", False),
        patch.object(settings, "enable_nightly_report_generation", False),
        patch.object(settings, "enable_kiis_iheart_web_collector", False),
...
def test_scheduler_all_enabled_has_nine_jobs() -> None:
    ...
    with (
        ...
        patch.object(settings, "enable_nova_radoxo_collector", True),
        patch.object(settings, "enable_nova_radio_australia_collector", True),
        patch.object(settings, "enable_capital_collector", True),
        patch.object(settings, "enable_capital_ukradiolive_collector", True),
        patch.object(settings, "enable_nightly_reconciliation", True),
        patch.object(settings, "enable_nightly_report_generation", True),
        patch.object(settings, "enable_kiis_iheart_web_collector", True),
        ...
        assert ids == {
            "nova_daily_diary",
            "nova_radoxo_playlist",
            "nova_radio_australia_chart",
            "capital_now_playing",
            "capital_ukradiolive_playlist",
            "nightly_reconciliation",
            "nightly_report_generation",
            "kiis_iheart_web_playlist",
...
def test_nova_radoxo_job_uses_interval_trigger() -> None:
    from app.core.settings import settings
    with patch.object(settings, "enable_nova_radoxo_collector", True):
        sched = build_scheduler()
        job = sched.get_job("nova_radoxo_playlist")
        assert job is not None
        assert isinstance(job.trigger, IntervalTrigger)


def test_nova_radio_australia_job_uses_cron_trigger() -> None:
    from app.core.settings import settings
    with patch.object(settings, "enable_nova_radio_australia_collector", True):
        sched = build_scheduler()
        job = sched.get_job("nova_radio_australia_chart")
        assert job is not None
        assert isinstance(job.trigger, CronTrigger)
...
def test_capital_ukradiolive_job_uses_interval_trigger() -> None:
    from app.core.settings import settings
    with patch.object(settings, "enable_capital_ukradiolive_collector", True):
        sched = build_scheduler()
        job = sched.get_job("capital_ukradiolive_playlist")
        assert job is not None
        assert isinstance(job.trigger, IntervalTrigger)
...
def test_kiis_iheart_web_job_uses_interval_trigger() -> None:
    from app.core.settings import settings
    with patch.object(settings, "enable_kiis_iheart_web_collector", True):
        sched = build_scheduler()
        job = sched.get_job("kiis_iheart_web_playlist")
        assert job is not None
        assert isinstance(job.trigger, IntervalTrigger)
...
@pytest.mark.anyio
async def test_nova_radoxo_job_invokes_collector() -> None:
    import uuid
    ...
    _sched = "app.infrastructure.scheduler.scheduler"
    with (
        patch(f"{_sched}.NovaRadoxoCollector", return_value=mock_collector),
        patch(f"{_sched}._persist_result", new_callable=AsyncMock),
    ):
        await job_collect_nova_radoxo()

    mock_collector.run.assert_awaited_once()


@pytest.mark.anyio
async def test_nova_radio_australia_job_invokes_collector() -> None:
    import uuid
    ...
    _sched = "app.infrastructure.scheduler.scheduler"
    with (
        patch(f"{_sched}.RadioAustraliaOrgCollector", return_value=mock_collector),
        patch(f"{_sched}._persist_result", new_callable=AsyncMock),
    ):
        await job_collect_nova_radio_australia()
...
@pytest.mark.anyio
async def test_capital_ukradiolive_job_invokes_collector() -> None:
    import uuid
    ...
    _sched = "app.infrastructure.scheduler.scheduler"
    with (
        patch(f"{_sched}.CapitalUKRadioLiveCollector", return_value=mock_collector),
        patch(f"{_sched}._persist_result", new_callable=AsyncMock),
    ):
        await job_collect_capital_ukradiolive()
...
@pytest.mark.anyio
async def test_kiis_iheart_web_job_invokes_collector() -> None:
    import uuid
    ...
    _sched = "app.infrastructure.scheduler.scheduler"
    with (
        patch(f"{_sched}.KIISIHeartWebCollector", return_value=mock_collector),
        patch(f"{_sched}._persist_result", new_callable=AsyncMock),
    ):
        await job_collect_kiis_iheart_web()
