"""Tests for WKSC 103.5 Kiss FM Chicago iHeart now-playing and history collectors.

No live network calls. Uses saved JSON fixtures. Tests confirm WKSC-specific
station ID (821), URL construction, and end-to-end parse paths via the shared
IHeartNowPlayingCollector and IHeartRecentlyPlayedCollector generic classes.

VAL-WKSC-001: stream ID 821 is UNVALIDATED; confirm via DevTools on
https://1035kissfm.iheart.com/music/recently-played/ before enabling the collector.
"""
...
_FIXTURE_200 = Path(__file__).parent.parent.parent / "fixtures/json/iheart_wksc_200.json"
_FIXTURE_RECENT = (
    Path(__file__).parent.parent.parent / "fixtures/json/iheart_wksc_recently_played.json"
)


# ── Now-playing collector ─────────────────────────────────────────────────────

def test_now_playing_url_contains_821_and_current_track() -> None:
...
def test_now_playing_parse_fixture() -> None:
    ...
    assert len(plays) == 1
    assert no_tracks == []
    assert plays[0].raw_artist == "Olivia Rodrigo"
    assert plays[0].raw_title == "good 4 u"
    assert plays[0].attribution == "iheart"
    assert plays[0].source_event_id == "iheart-821-20260604-01230"
...
def test_now_playing_timestamp_is_utc_aware() -> None:
    ...
    assert int(plays[0].played_at.timestamp()) == 1748049000
...
def test_history_url_contains_821_and_recently_played() -> None:
...
def test_history_first_track_is_good_4_u() -> None:
    raw = _FIXTURE_RECENT.read_bytes()
    collector = IHeartRecentlyPlayedCollector(
        source_id=uuid.uuid4(), station_id=uuid.uuid4(), iheart_station_id="821"
    )
...
def test_history_all_source_event_ids_use_821() -> None:
    raw = _FIXTURE_RECENT.read_bytes()
    collector = IHeartRecentlyPlayedCollector(
        source_id=uuid.uuid4(), station_id=uuid.uuid4(), iheart_station_id="821"
    )
    ...
    for play in plays:
        ...
        assert "821" in play.source_event_id
