  ...
  container.innerHTML = `
    ...
    <div class="ops-section">
      ...
      <div class="ops-grid">
        ...
        ${opsCard('API Docs', ov.docs_exposed ? stateChip('Exposed', ov.is_production ? 'chip-warn' : 'chip-info') : stateChip('Hidden', 'chip-ok'), 'Interactive /docs endpoint. Hidden in production unless ENABLE_DOCS_IN_PRODUCTION=true.')}
        ${opsCard('Admin Auth', ov.admin_auth_enabled ? stateChip('Protected', 'chip-ok') : stateChip('Public', 'chip-info'), 'HTTP Basic Auth for the /admin console. Configured via ADMIN_BASIC_AUTH_USER and ADMIN_BASIC_AUTH_PASSWORD.')}
    ...
    <div class="ops-section">
      ...
      <div class="card">
        ...
        <table class="flag-table">
          <tbody>
            ${collectorRow('Capital FM UK',           ov.collector_flags.capital,                'ENABLE_CAPITAL_COLLECTOR')}
            ${collectorRow('Nova 96.9 FM',             ov.collector_flags.nova,                   'ENABLE_NOVA_COLLECTOR')}
            ${collectorRow('KIIS FM 102.7',            ov.collector_flags.kiis,                   'ENABLE_KIIS_COLLECTOR')}
            ${collectorRow('Nightly Reconciliation',   ov.collector_flags.nightly_reconciliation, 'ENABLE_NIGHTLY_RECONCILIATION')}
          </tbody>
        </table>
      </div>
    </div>
    ...
    <div class="ops-section">
      ...
      ${!ov.admin_auth_enabled
        ...
        : ''}
      ${ov.docs_exposed && ov.is_production
...
function envValue(ov) {
  const label = ov.environment || 'unknown';
  const cls   = ov.is_production ? 'env-badge env-prod' :
...
function envDesc(ov) {
  if (ov.is_production) return 'Production environment. All safety defaults apply.';
  if (ov.environment === 'staging') return 'Staging environment. Treat as production-like.';
  return 'Development environment. Docs are exposed and auth is optional.';
}

function retentionValue(ov) {
  if (ov.retention_enabled) {
