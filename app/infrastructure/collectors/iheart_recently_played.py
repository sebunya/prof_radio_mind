"""Generic iHeart recently-played batch collector — reusable for any iHeart station.

Polls GET https://api.iheart.com/api/v3/live-meta/stream/{id}/recentlyPlayed
once per hour. Acts as a historical fallback when the 5-minute now-playing
poll misses a short track. Caller supplies iheart_station_id — no per-station
subclass needed.
"""
...
class IHeartRecentlyPlayedCollector(BaseCollector):
    """Collects iHeart recently-played tracks for any station (configurable station ID).
    ...
    def _build_url(self) -> str:
        return f"{self.base_url}/{self.iheart_station_id}/recentlyPlayed"
    ...
    def parse(
        ...
        if http_status == 204:
            ev = NoTrackEvent.create(
                ...
                raw_http_status=204,
                notes=f"iHeart recentlyPlayed HTTP 204 — station {self.iheart_station_id}",
            )
            return [], [ev]

        results = parse_iheart_recently_played(raw_bytes, http_status)
        plays = [
            PlayEvent.create(
                station_id=self.station_id,
                source_id=self.source_id,
                ...
                attribution="iheart_recently_played",
            )
            for r in results
        ]
        return plays, []
