"""Capital FM iHeart Now Playing collector.

Same protocol as KIIS-FM iHeart. Station ID is config-driven and
must be confirmed via VAL-CAP-001 before enabling in production.

CRITICAL BEHAVIOUR (shared with KIISIHeartCollector):
...
# Placeholder station ID — must be confirmed via VAL-CAP-001
_DEFAULT_STATION_ID = "capitalfm"
_REQUEST_TIMEOUT = 15.0


class CapitalIHeartCollector(BaseCollector):
    """Collects Capital FM now-playing data from the iHeart API."""
    ...
    def parse(
        ...
        if http_status == 204:
            ev = NoTrackEvent.create(
                ...
                raw_http_status=204,
                notes="Capital FM iHeart HTTP 204 — commercial break or talk",
            )
            return [], [ev]
